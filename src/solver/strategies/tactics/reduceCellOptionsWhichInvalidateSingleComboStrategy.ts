...
export class ReduceCellOptionsWhichInvalidateSingleComboStrategy extends Strategy {
    ...
    execute() {
        ...
        _.range(0, GridSizeAndCellPositionsIteration.GRID_SIDE_CELL_COUNT).forEach((row: HouseIndex) => {
            _.range(0, GridSizeAndCellPositionsIteration.GRID_SIDE_CELL_COUNT).forEach((col: HouseIndex) => {
