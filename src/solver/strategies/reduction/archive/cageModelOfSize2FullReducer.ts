        ...
        for (const combo of cageMCombos.combos) {
            ...
            if (!cell0HasNum0) {
                if (!cell1HasNum0) {
                    ...
                    // ```
                    //
                    if (cell0HasNum1) reduction.deleteNumOpt(cellM0, num1, cageM);
                    if (cell1HasNum1) reduction.deleteNumOpt(cellM1, num1, cageM);
                    cageMCombos.deleteCombo(combo);
                } else if (cell1HasNum1) {
                    //
                    // If the first `CellModel` does *not* have the first `Combo` number
            ...
            if (!cell0HasNum1) {
                if (!cell1HasNum1) {
                    ...
                    // ```
                    //
                    if (cell0HasNum0) reduction.deleteNumOpt(cellM0, num0, cageM);
                    if (cell1HasNum0) reduction.deleteNumOpt(cellM1, num0, cageM);
                    cageMCombos.deleteCombo(combo);
                } else if (cell1HasNum0) {
                    //
                    // If the first `CellModel` does *not* have the second `Combo` number
