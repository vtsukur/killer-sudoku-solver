 ...
 * all possible {@link Cell} indices on the {@link Grid} (see {@link GridMatrix.CELL_COUNT}).
 *
...
export class CellIndicesCheckingSet implements
    ...
    private static readonly _CELL_INDEX_TO_BIT_STORE_LOCATORS: ReadonlyArray<CellIndexToBitStoreLocator> = GridMatrix.CELL_INDICES_RANGE.map(cellIndex => {
    ...
    private static readonly _BITS_TO_CELLS_LUT: ReadonlyArray<ReadonlyArray<Cell>> = (() => {
        ...
        for (const index of GridMatrix.CELL_INDICES_RANGE) {
            //
            ...
            //
            const row = ~~(index / GridMatrix.SIDE_CELL_COUNT);

            // Calculating index of the `Cell` `Column` from absolute `Cell` index on the `Grid`.
            const col = index % GridMatrix.SIDE_CELL_COUNT;
