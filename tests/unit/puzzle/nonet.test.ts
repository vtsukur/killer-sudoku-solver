...
describe('Nonet tests', () => {
    ...
    test('Iteration over Cells', () => {
        const houseCells = new Array(House.SIZE);
        _.range(House.COUNT_OF_ONE_TYPE).forEach((nonet) => {
            houseCells[nonet] = Array.from(Nonet.cellsIterator(nonet));
        });

        // north-west nonet (0)
        expect(houseCells[0]).toEqual([
