    ...
    test('Comparable test for 2 `Combo`s, 5 present numbers and 11 deleted numbers', () => {
        runComparablePerformanceTests({
            ...
            prepareForReduction: (cageM, reduction) => {
                ...
                const allCombos = cageM.combosSet.combos;
                cageM.combosSet.deleteComboFailSafe(allCombos.find(combo => combo.has(2)) as Combo);
                cageM.combosSet.deleteComboFailSafe(allCombos.find(combo => combo.has(3)) as Combo);
                expect(Array.from(cageM.combosSet.combos)).toEqual([
                    Combo.of(1, 8),
                    Combo.of(4, 5)
                ]);
