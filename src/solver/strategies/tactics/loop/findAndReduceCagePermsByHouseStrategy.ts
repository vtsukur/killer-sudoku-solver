...
export class FindAndReduceCagePermsByHouseStrategy extends Strategy {
    ...
    execute() {
        ...
        this._model.houseModels.forEach(houseM => {
            _.range(1, House.CELL_COUNT_RANGE_INCSLUSIVE_UPPER_BOUND).forEach((num: number) => {
                ...
                if (!_.isUndefined(singleCellForNum)) {
                    ...
                    reduction.tryReduceNumOpts(singleOptionCellM, SudokuNumsSet.ofNum(num));
                }
        ...
        for (const cageM of this._model.cageModelsMap.values()) {
            ...
            for (const numPlacementClue of cageM.findNumPlacementClues()) {
                if (!_.isUndefined(numPlacementClue.row)) {
                    reduceByHouse(cageM, this._model.rowModels[numPlacementClue.row], this._model, Combo.ofNum(numPlacementClue.num), reduction);
                } else if (!_.isUndefined(numPlacementClue.col)) {
                    reduceByHouse(cageM, this._model.columnModels[numPlacementClue.col], this._model, Combo.ofNum(numPlacementClue.num), reduction);
                }

                if (!_.isUndefined(numPlacementClue.nonet)) {
                    reduceByHouse(cageM, this._model.nonetModels[numPlacementClue.nonet], this._model, Combo.ofNum(numPlacementClue.num), reduction);
                }
            }
        }
        ...
        this._model.nonetModels.forEach(nonetM => {
            ...
            _.range(1, House.CELL_COUNT_RANGE_INCSLUSIVE_UPPER_BOUND).forEach(num => numMap.set(num, {
                rows: new Set(),
                cols: new Set()
            }));
            ...
            _.range(1, House.CELL_COUNT_RANGE_INCSLUSIVE_UPPER_BOUND).forEach(num => {
                const entry = numMap.get(num);
